...
Derivare dei dati discontinui porta sempre, o quasi sempre, ad avere delle curve spezzate, per le quali è difficile trovare dei valori affidabili per fare \todo[inlinepar]{parola che non mi viene, tipo "ragionamento" o qualche cosa simile. forse: "trarre informazioni utili"}.
Questo è stato il caso per la transconduttanza; la figura \ref{fig:gm_smooth} (la curva di sinistra) \todo[inlinepar]{da mettere figura con sotto figure} rappresenta la $g_m$ di un transistor  MOSFET a canale N, con $ W = 100\mu m$ e $L = 30 nm$ senza eseguire nessun tipo di smooth, in particolare per alte $V_{GS}$, si nota particolarmente questo difetto.

\begin{figure}[t]
    \centering
    % W = 100 
    \includegraphics[width=0.49\textwidth]{./capitolo2/transconduttanza/estrazione_dati/gm_no_smooth.png}
    \includegraphics[width=0.49\textwidth]{./capitolo2/transconduttanza/estrazione_dati/gm_smooth.png}

    \caption[Confronto $g_m$ con utilizzo di smooth e senza]{Confronto $g_m$ con utilizzo di smooth e senza}
    \label{fig:gm_smooth}
\end{figure}
...
Uno dei effetti delle radiazioni ionizzanti è quella di ridurre la mobilità dei portatori di carica $\mu$ all'interno del canale. Osservando  l'espressione della $g_m$ si nota la proporzionalità diretta con la mobilità dei portatori comportando una riduzione della transconduttanza, all'aumentare della dose assorbita. Effetto confermato dai dati sperimentali (vedi tabelle \ref{tab:deltaGm_N} e \ref{tab:deltaGm_P} e figura \ref{fig:delta_gm}) ottenuti dai transistori sotto osservazione in questo lavoro di tesi.
