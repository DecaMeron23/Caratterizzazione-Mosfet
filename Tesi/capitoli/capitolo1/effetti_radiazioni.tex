...
\end{itemize}
Mentre il danno da spostamento (\textit{DD}) non è molto rilevante sui MOSFET, la ionizzazione può comportare variazioni dei parametri elettrici come: guadagno e tensione di soglia; per questo motivo ci concentreremo solo su gli effetti della ionizzazione.
...
Al contrario le lacune sono molto meno mobili, dai 5 ai 12 ordini di grandezza inferiori, rispetto agli elettroni. La maggior parte delle lacune riesce a sopravvive alla ricombinazione con gli elettroni, creando perciò una carica positiva vicino alla giunzione $Si/SiO_2$. Un aumento della carica positiva ha come effetto di diminuire la tensione di soglia del MOSFET a canale P; invece negli NMOS, dopo aver subito bassi dosaggi (inferiori a $10Mrad$), si può notare un abbassamento della tensione di soglia mentre, per dosi superiori, un innalzamento. Questo andamento è dato da due fattori:
\begin{enumerate}
...
Gli effetti precedentemente elencati, in particolare sui MOSFET a canale N, avevano portato ad una preferenza di utilizzo per i PMOS essendo più tolleranti alle radiazioni ionizzanti. Ma l'evoluzione di tecnologie MOSFET sempre più piccole (figura \ref{fig:scaling_mosfet}), ha comportato ad un aumento della resistenza alle radiazioni ionizzanti, riducendo correnti di perdita, $I_{off}$ e variazioni della tensione di soglia.  
...
La \textit{TID} normalmente è misurata in $rad$ (\textit{radiation absorbed dose}), definito come $100 erg  = 100 \cdot 10^{-7} J$ per grammo di materiale, oppure, accolto dal sistema internazionale, in \textit{gray} ($Gy$) dove $1 Gy = 100rad = 1\frac{J}{Kg}$.
